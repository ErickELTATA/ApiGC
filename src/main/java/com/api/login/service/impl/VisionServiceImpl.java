...
import com.api.login.dao.VisionDao;
...
public class VisionServiceImpl implements VisionService {
    ...
    public ResponseEntity<String> register(Map<String, String> requestMap) {
        ...
        try {
            if (validateRegister(requestMap)){
                ...
                visionDao.save(getMisionFromMap(requestMap));
    ...
    public ResponseEntity<String> update(Integer id,Map<String, String> requestMap ) {
        try {
            ...
            Optional<Vision> optionalUser = visionDao.findById(id);

            if (!optionalUser.isEmpty()) {
                // Obtén el usuario de la base de datos
                Vision vision = optionalUser.get();

                // Actualiza los campos del usuario con los valores proporcionados en el mapa

                vision.setFecha(Date.valueOf(requestMap.get("fecha").toString()));
                vision.setContenido(requestMap.get("contenido"));


                // Actualiza el usuario en la base de datos
                visionDao.save(vision);
    ...
    public ResponseEntity<String> delete(Integer userId) {
        try {
            ...
            Optional<Vision> optionalUser = visionDao.findById(userId);

            if (optionalUser.isPresent()) {
                // Elimina al usuario de la base de datos
                visionDao.deleteById(userId);
    ...
    @Override
    public ResponseEntity<List<Vision>> getAllResena() {
        try{
            return new ResponseEntity<>(visionDao.getAllVision(),HttpStatus.OK);
        }catch (Exception exception){
            exception.printStackTrace();
        }
    ...
    private Vision getMisionFromMap(Map<String, String> requestMap){
        Vision vision = new Vision();
        vision.setFecha(Date.valueOf(requestMap.get("fecha")));
        vision.setContenido(requestMap.get("contenido"));
        return vision;
    }
